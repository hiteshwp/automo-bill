                ...
                <div class="card">
                    <div class="card-body">
                        <div class="page-title-box p-0 d-sm-flex align-items-center justify-content-between">
                            <h4 class="mb-sm-0 d-flex align-items-center gap-2"><a href="{{ route('garage-owner.booking.list') }}" class="btn btn-soft-primary btn-icon shadow-none"><i class="ri-arrow-left-line"></i></a> Update Estimate - <span class="fs-14 fw-normal">({{ $estimate_data->client_name }})</span></h4>
                    ...
                    <div class="card-body">
                        <div class="row justify-content-center">
                            <div class="col-12 col-md-8 col-lg-9">
                                <form method="post" id="frmeditestimateinformation">
                                            ...
                                            <div class="flex-grow-1">
                                                ...
                                                <div class="mt-4">
                                                    ...
                                                    <div class="formgroup mb-3 custom-width1">
                                                        <input type="text" class="form-control" data-provider="flatpickr" name="txtestimatedate" data-date-format="Y-m-d" placeholder="Select date" value="{{ $estimate_data->estimate_date }}" />
                                                    </div>
                                                </div>
                                            </div>
                                            @php
                                    ...
                                    <div class="invoice-user-info-list py-4">
                                        <div class="invoice-user-info-item">
                                            ...
                                            <h5 class="fs-14 mb-0"><span id="address">{{ $estimate_data->client_id }}</span></h5>
                                        </div>
                                        <div class="invoice-user-info-item">
                                        ...
                                        <div class="invoice-user-info-item">
                                            ...
                                            <h5 class="fs-14 mb-0"><span id="invoice-date">{{ date("d M, Y", strtotime($estimate_data->booking_date)) }}</span> <small class="text-muted" id="invoice-time">{{ date("h:iA", strtotime($estimate_data->booking_date)) }}</small></h5>
                                        </div>
                                        <div class="invoice-user-info-item">
                                    ...
                                    <div class="invoice-form-block">
                                        ...
                                        <div class="invoice-note-block mt-4">
                                            <div class="formgroup">
                                                ...
                                                <textarea class="form-control resize-none" rows="5" placeholder="Enter your note">{{ $estimate_data->estimate_notes }}</textarea>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="invoice-total-info py-4 px-0">
                                        <div class="border-top border-top-dashed mt-2">
                                            <table class="table table-borderless table-nowrap align-middle mb-0 ms-auto" style="width:250px">
                                                <tbody>
                                                    <tr>
                                                        <td>Total Labour</td>
                                                        <td class="text-end" id="txttotallabour">${{ $estimate_data->estimate_labor_total }}</td>
                                                    </tr>
                                                    <tr>
                                                        <td>Total Parts/Materials</td>
                                                        <td class="text-end" id="txttotalpartsmaterials">${{ $estimate_data->estimate_parts_total }}</td>
                                                    </tr>
                                                    <tr>
                                                        <td>Tax</td>
                                                        <td class="text-end" id="txttotaltax"> {{ $estimate_data->estimate_tax }}</td>
                                                    </tr>
                                                    <tr class="border-top border-top-dashed fs-15">
                                                        <th scope="row">Total Due Amount</th>
                                                        <th class="text-end" id="txttotalduewamount">${{ $estimate_data->estimate_total_inctax }}</th>
                                                    </tr>
                                                </tbody>
                                            </table>
                                            <!--end table-->
                                        ...
                                        <div class="hstack gap-2 justify-content-end d-print-none mt-4">
                                            ...
                                            <div class="formgroup form-check ps-0">
                                                <div class="formgroup">
                                                    <select class="form-select pe-5" aria-label="Default select example" required="" name="txtestimatestatus">
                                                        <option value="pending" {{ $estimate_data->estimate_carOwnerApproval == "pending" ? 'selected' : '' }}>Pending</option>
                                                        <option value="approved" {{ $estimate_data->estimate_carOwnerApproval == "approved" ? 'selected' : '' }}>Approved</option>
                                                        <option value="rejected" {{ $estimate_data->estimate_carOwnerApproval == "rejected" ? 'selected' : '' }}>Rejected</option>
                                                    </select>
                                                </div>
                                            </div>
                                            @if ( $estimate_data->estimate_carOwnerApproval != "approved" )
                                                <button type="submit" class="btn btn-success" id="btnupdateestimate">Update</button>
                                            @endif
                                            <input type="hidden" name="txtsumtotallabour" id="txtsumtotallabour" value="{{ $estimate_data->estimate_labor_total }}"/>
                                            <input type="hidden" name="txtsumtotalparts" id="txtsumtotalparts" value="{{ $estimate_data->estimate_parts_total }}"/>
                                            <input type="hidden" name="txtsumtotaltax" id="txtsumtotaltax" value="{{ $estimate_data->estimate_tax }}"/>
                                            <input type="hidden" name="txtsumtotaldueamountexcepttax" id="txtsumtotaldueamountexcepttax" value="{{ $estimate_data->estimate_total }}"/>
                                            <input type="hidden" name="txtsumtotaldueamount" id="txtsumtotaldueamount" value="{{ $estimate_data->estimate_total_inctax }}"/>
                                            <input type="hidden" name="txtbookingid" id="txtbookingid" value="{{ $estimate_data->estimate_booking_id }}"/>
                                            <input type="hidden" name="txtcustomerid" id="txtcustomerid" value="{{ $estimate_data->estimate_customer_id }}"/>
                                            <input type="hidden" name="txtvehicleid" id="txtvehicleid" value="{{ $estimate_data->estimate_vehicle_id }}"/>
                                            <input type="hidden" name="txtestimateid" id="txtestimateid" value="{{ $estimate_data->estimate_id }}"/>
                                        </div>
                                    </div>
                                </form>
                            </div><!--end col-->
